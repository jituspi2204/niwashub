...
const Empty: React.FC<Props> = ({ title, subTitle, style }) => (
  <View
    style={[
      {
        flex: 1,
    ...
    <View style={[{ width: 200, height: 190 }]}>
      <Svg
    ...
    <View>
      {title && (
        <Text h6 n800 style={{ textAlign: 'center', marginTop: 12 }}>
          {title}
        </Text>
      )}
      {subTitle && (
        <Text
          base2Medium
          n600
          style={{ textAlign: 'center', marginBottom: 12, width: '50%' }}>
          {subTitle}
        </Text>
      )}
    </View>
